...
@description('Optional existing Azure AI Search service name when useExistingAiProject=true')
...
var hasSearch = contains(map(aiProjectDependentResources, r => r.resource), 'azure_ai_search')
var hasStorage = contains(map(aiProjectDependentResources, r => r.resource), 'storage')
var dependentResourceToken = uniqueString(subscription().id, resourceGroupName, location, 'hosted-deps')
var autoSearchConnectionName = 'search-${dependentResourceToken}'
var autoStorageConnectionName = 'storage-${dependentResourceToken}'
...
var dependentResourcesWithSearch = (enableHostedAgents && !hasSearch) ? union(aiProjectDependentResources, [
  {
    resource: 'azure_ai_search'
    connectionName: autoSearchConnectionName
  }
]) : aiProjectDependentResources
var dependentResourcesBase = (enableHostedAgents && !hasSearch && !hasStorage) ? union(dependentResourcesWithSearch, [
  {
    resource: 'storage'
    connectionName: autoStorageConnectionName
  }
]) : dependentResourcesWithSearch
var dependentResources = shouldCreateAcr ? union(dependentResourcesBase, [
  {
    resource: 'registry'
    connectionName: 'acr-${uniqueString(subscription().id, resourceGroupName, location)}'
  }
]) : dependentResourcesBase
...
// Search + storage for existing Foundry project when no search service name is supplied
var shouldProvisionSearchForExisting = useExistingAiProject && enableHostedAgents && empty(existingSearchServiceName)
var existingSearchResourceToken = uniqueString(subscription().id, resourceGroupName, location, 'existing-search')
var existingSearchConnectionName = 'search-${existingSearchResourceToken}'
var existingStorageConnectionName = 'storage-${existingSearchResourceToken}'

module storageForExistingProject 'core/storage/storage.bicep' = if (shouldProvisionSearchForExisting) {
  scope: rg
  name: 'storage-for-existing-project'
  params: {
    location: location
    tags: tags
    resourceName: 'st${uniqueString(subscription().id, resourceGroupName, location)}'
    connectionName: existingStorageConnectionName
    principalId: principalId
    principalType: principalType
    aiServicesAccountName: aiFoundryResourceName
    aiProjectName: aiFoundryProjectName
...
module searchForExistingProject 'core/search/azure_ai_search.bicep' = if (shouldProvisionSearchForExisting) {
  scope: rg
  name: 'search-for-existing-project'
  params: {
    tags: tags
    resourceName: 'search-${existingSearchResourceToken}'
    connectionName: existingSearchConnectionName
    storageAccountResourceId: storageForExistingProject.outputs.storageAccountId
...
// Azure AI Search
var searchServiceNameOutput = shouldProvisionSearchForExisting
  ? searchForExistingProject.outputs.searchServiceName
  : (useExistingAiProject ? existingAiProject.outputs.dependentResources.search.serviceName : aiProject.outputs.dependentResources.search.serviceName)
var searchConnectionNameOutput = shouldProvisionSearchForExisting
  ? searchForExistingProject.outputs.searchConnectionName
  : (useExistingAiProject ? existingAiProject.outputs.dependentResources.search.connectionName : aiProject.outputs.dependentResources.search.connectionName)
output AZURE_AI_SEARCH_CONNECTION_NAME string = searchConnectionNameOutput
output AZURE_AI_SEARCH_SERVICE_NAME string = searchServiceNameOutput

// Model / embedding / index names derived from aiProjectDeployments when present (single source with azure.yaml)
var embeddingDeployments = filter(aiProjectDeployments, d => contains(toLower(d.model.name), 'embedding') || contains(toLower(d.name), 'embedding'))
var chatDeployments = filter(aiProjectDeployments, d => !(contains(toLower(d.model.name), 'embedding') || contains(toLower(d.name), 'embedding')))
var resolvedChatDeployment = length(chatDeployments) > 0 ? chatDeployments[0].name : 'gpt-4.1-mini'
var resolvedEmbeddingDeployment = length(embeddingDeployments) > 0 ? embeddingDeployments[0].name : 'text-embedding-3-small'
var defaultSearchIndexName = 'az204-certification-index'
output MODEL_DEPLOYMENT_NAME string = resolvedChatDeployment
output AZURE_AI_MODEL_DEPLOYMENT_NAME string = resolvedChatDeployment
output AZURE_AI_EMBEDDING_DEPLOYMENT_NAME string = resolvedEmbeddingDeployment
output AZURE_AI_SEARCH_INDEX_NAME string = defaultSearchIndexName
output AZURE_AI_SEARCH_ENDPOINT string = !empty(searchServiceNameOutput) ? 'https://${searchServiceNameOutput}.search.windows.net' : ''
